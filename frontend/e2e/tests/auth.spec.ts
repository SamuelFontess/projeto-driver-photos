...
test.describe('Login', () => {
  ...
  test('botão de submit fica desabilitado enquanto submete', async ({ page }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
  ...
  test('exibe erro com credenciais inválidas', async ({ page }) => {
    ...
    await expect(page.getByText('Erro ao fazer login', { exact: true })).toBeVisible();
  });
  ...
  test('redireciona para /dashboard após login bem-sucedido', async ({ page }) => {
    ...
    await page.route('**/api/folders**', (route) =>
      route.fulfill({ status: 200, body: JSON.stringify({ folders: [] }) })
    );
    await page.route('**/api/files**', (route) =>
      route.fulfill({ status: 200, body: JSON.stringify({ files: [] }) })
    );
...
test.describe('Registro', () => {
  ...
  test('redireciona para /dashboard após registro bem-sucedido', async ({ page }) => {
    const registerPage = new RegisterPage(page);
    await registerPage.goto();
    ...
    await registerPage.fillAndSubmit('Test User', 'test@driver.com', 'senha123');
  ...
  test('exibe erro se o email já existe', async ({ page }) => {
    ...
    await registerPage.fillAndSubmit('Outro User', 'existe@driver.com', 'senha123');

    await expect(page.getByText('Erro ao criar conta', { exact: true })).toBeVisible();
  });
...
test.describe('Recuperação de senha', () => {
  test('exibe o formulário de recuperação', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /recuperar|senha|Forgot/i })).toBeVisible();
